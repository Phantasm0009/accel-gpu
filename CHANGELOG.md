...
- **Package author metadata** — added `author: "Aditya Tiwari"` in package manifest

### Changed

- **Playwright backend consistency stability** — WebKit+WebGL `matmul` drift remains logged for visibility, while required backend consistency gates continue to enforce stable checks across other critical operations
